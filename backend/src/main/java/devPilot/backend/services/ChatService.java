...
public class ChatService {
    ...
    public SseEmitter streamReply(UUID userId, UUID sessionId, String userContent) {
        // 1. Ensure the session exists and the repo is indexed (BEFORE creating SseEmitter)
        ChatSession session = requireSession(userId, sessionId);
        ...
        // 5. NOW create and stream OpenAI response to the client (SSE)
        // All validation is complete, so we can safely return the emitter
